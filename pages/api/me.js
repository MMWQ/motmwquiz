...
export default async function handler(req, res) {
    ...
    if (resp.rowCount === 0) {
        ...
        res.status(200).json({ email: session.user.email, name, ums_front: 'map', ums_back: 'state', os_front: 'state', os_back: 'capital', ut_front: 'territory', ut_back: 'region', ums: (new Array(193).fill("1")).join(""), os: (new Array(2).fill("1")).join(""), ut: (new Array(5).fill("1")).join("") });
    }
